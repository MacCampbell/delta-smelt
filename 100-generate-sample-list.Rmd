...
```{r}
tops<-reads %>% top_frac(.9, Counts) %>% filter(sex!=0)
tops %>% group_by(Pheno, sex) %>% summarize(Count=n())
```
...
sex<-tops %>% select(sex)
write_tsv(sex, "phenos/2012.cov")
```


## doAsso
With and without sex as cov ay 75% thresh    

```{sh, eval=FALSE}
srun -p high -t 48:00:00 --mem=32G --nodes=2 $HOME/angsd/angsd -P 24  -bam $HOME/delta-smelt/bamlists/2012.bamlist \
-yBin $HOME/delta-smelt/phenos/2012.phenos -cov $HOME/delta-smelt/phenos/2012.cov -minMapQ 20 -minQ 20 -minInd 109 -doAsso 2 \ 
-doPost 1 -GL 1 -minCount 2 -out $HOME/delta-smelt/outputs/2012/assoc05-75-2 -doMajorMinor 1 -doMaf 1 -SNP_pval 1e-6 \
...
srun -p high -t 48:00:00 --mem=32G --nodes=2 $HOME/angsd/angsd -P 24  -bam $HOME/delta-smelt/bamlists/2012.bamlist \
-yBin $HOME/delta-smelt/phenos/2012.phenos -minMapQ 20 -minQ 20 -minInd 109 -doAsso 2 -doPost 1 -GL 1 -minCount 2 \
